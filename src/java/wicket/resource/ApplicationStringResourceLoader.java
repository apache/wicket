...
public class ApplicationStringResourceLoader implements IStringResourceLoader {
    ...
    private static final Log code = LogFactory.getLog(Page.class);
     ...
     */
    private synchronized ValueMap loadResources(final String style, final Locale locale, final String id) {
        ...
        if ( resource != null ) {
            try {
                ...
                code.warn("Unable to find resource " + resource, e);
                strings = ValueMap.EMPTY_MAP;
            } catch (IOException e) {
                code.warn("Unable to access resource " + resource, e);
                strings = ValueMap.EMPTY_MAP;
            }
        } else {

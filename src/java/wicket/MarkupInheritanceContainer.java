/*
 * $Id: MarkupInheritanceContainer.java,v 1.6 2005/07/29 15:13:06 jdonnerstag
 * Exp $ $Revision$ $Date$
 * 
 * ==============================================================================
...
public class MarkupInheritanceContainer extends WebMarkupContainer implements IComponentResolver
{
	 ...
	 * 
	 * @param parent
	 *            Not the inheritance container but its parent component
	...
	private MarkupStream getInheritedMarkupStream(MarkupContainer parent)
	{
		...
		return getApplication().getMarkupCache().getMarkupStream(parent,
	...
	{
		// Not sure this can really happen, but onComponentTagBody() must be
		// called first
		if (openTag == null)
		{
			return false;
		...
		{
			markupStream
					.throwMarkupException("A <wicket:child> tag must be an open or open-close tag.");
		}
		...
		log.debug("inherit: render body (<wicket:child>): "
				+ dual.getCurrentMarkupStream().toString());
		super.onComponentTagBody(this.dual, openTag);
		this.dual.decrementMarkupStreamIndex();
		log.debug("inherit: back from render </wicket:child>: "
				+ dual.getCurrentMarkupStream().toString());

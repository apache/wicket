...
/**
 * Lightweight document parser for HTML. This parser is only intended
 * to process well formed and simple HTML of the kind that would generally
 * be utilised during testing.
 *
 * @author Chris Turner
 */
public class HtmlDocumentParser {

    // Constants for parser tokens
        ...
        for ( int i = 0; i < attributeElements.length; i++ ) {
            ...
            if ( bits.length == 1 ) {
                ...
                for ( int j = 1; j < bits.length; j++ ) {
                    value.append(bits[j]);
                    if ( j < (bits.length - 1) ) value.append('=');
                }
                bits[1] = value.toString().trim();
                if ( bits[1].startsWith("\"") ) bits[1] = bits[1].substring(1);
                if ( bits[1].endsWith("\"") ) bits[1] = bits[1].substring(0, bits[1].length() - 1);

...
;(function (undefined) {
	...
	Wicket.Palette.add=function(choicesId, selectionId, recorderId) {
		const choices=Wicket.Palette.$(choicesId);
		const selection=Wicket.Palette.$(selectionId);

		if (Wicket.Palette.moveHelper(choices, selection)) {
			const recorder=Wicket.Palette.$(recorderId);
	...
	Wicket.Palette.updateRecorder=function(selection, recorder) {
		...
		Wicket.Event.fire(recorder, 'change');
	};
	...
	Wicket.Palette.addAll = function(choicesId, selectionId, recorderId) {
		const palette = Wicket.Palette;
		const choices = palette.$(choicesId);
		const selection = palette.$(selectionId);

		if (palette.moveAllHelper(choices, selection)) {
			const recorder = palette.$(recorderId);

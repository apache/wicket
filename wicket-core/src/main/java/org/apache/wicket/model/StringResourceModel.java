		...
		IComponentAssignedModel<String>
{
	...
	protected String getString(final Component component)
	{
		...
		if (parameters == null || parameters.length == 0)
		{
			...
			value = localizer.resolveStringWithLocalizationSupport(getResourceKey(), component, model, null, null, defaultValue);
		}
		else
		{
			// Get the string resource, doing not any property substitutions
			// that has to be done later after MessageFormat
			value = localizer.resolveStringWithLocalizationSupport(getResourceKey(), component, null, null, null, defaultValue);
			if (value != null)
			{

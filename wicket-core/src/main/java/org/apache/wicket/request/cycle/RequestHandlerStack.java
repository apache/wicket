...
public abstract class RequestHandlerStack
{
	...
	public void execute(final IRequestHandler handler)
	{
		...
		catch (RuntimeException exception)
		{
			...
			if (replacer.isRemoveAll() && !first)
			{
				throw exception;
			}
			replacementHandler = replacer.getReplacementRequestHandler();
		}
		finally
		{
	...
	public final IRequestHandler resolveHandler(RuntimeException exception)
	{
		...
		ReplaceHandlerException replacer = Exceptions.findCause(exception,
			ReplaceHandlerException.class);
		return replacer != null ? replacer.getReplacementRequestHandler() : null;
	}
	...
	public void detach()
	{
		...
		Set<Class<? extends IRequestablePage>> detachedPages = new HashSet<>();
		for (IRequestHandler handler : inactiveRequestHandlers)
		{
			...
			if (pageClass == null)
			{
				internalDetach(handler);
			}
			else if (!detachedPages.contains(pageClass))
			{
				detachedPages.add(pageClass);
				internalDetach(handler);
			}
		}
	}

	private void internalDetach(IRequestHandler handler)
	{
		try
		{
			detach(handler);

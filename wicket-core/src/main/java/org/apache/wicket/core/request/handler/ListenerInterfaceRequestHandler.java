		...
		ILoggableRequestHandler
{
	...
	public void respond(final IRequestCycle requestCycle)
	{
		...
		if (freshPage && pageComponentProvider.getPageId() != null && (isStateless == false || component == null))
		{
			// A listener interface is invoked on an expired page.

			// If the page is stateful then we cannot assume that the listener
			// interface is
			// invoked on its initial state (right after page initialization)
			// and that its
		...
		{
			// schedule page render after current request handler is done. this
			// can be

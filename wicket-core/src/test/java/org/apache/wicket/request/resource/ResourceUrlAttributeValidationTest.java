...
public class ResourceUrlAttributeValidationTest extends WicketTestCase
{
	...
	private static final String PACKAGE_PREFIX =
		ResourceUrlAttributeValidationTest.class.getPackage().getName().replace('.', '/') + "/";
	...
	private static final int PARENT_STEPS =
		ResourceUrlAttributeValidationTest.class.getPackage().getName().split("\\.").length + 2;
	...
	private static final String STYLE_WITH_SEPARATORS =
		"en-" + repeat("..%2F", PARENT_STEPS) + OUTSIDE_NAME.replace("-", "~").replace("/", "%2F");
	...
	/** {@code String#repeat} is not available on this branch. */
	 ...
	 */
	@Test
	public void unstyledResourceIsStillLocatedWhenStyleIsRejected()
	{
		RecordingFinder finder = install(new RecordingFinder());

		tester.executeUrl(URL);

		assertTrue("the unstyled resource should still be looked up; asked: " + finder.asked,
			finder.asked.contains(PACKAGE_PREFIX + "PublicPage.html"));
	}
	 ...
	 */
	@Test
	public void pathFromStyleStaysWithinPackage()
	{
		RecordingFinder finder = install(new RecordingFinder());

		tester.executeUrl(URL);

		assertFalse("no path should contain a parent reference; asked: " + finder.asked,
			finder.asked.stream().anyMatch(path -> path.contains("..")));
		assertTrue("every path should start with " + PACKAGE_PREFIX + "; asked: " + finder.asked,
			finder.asked.stream().allMatch(path -> path.startsWith(PACKAGE_PREFIX)));
	}
	 ...
	 */
	@Test
	public void pathFromLocaleStaysWithinPackage()
	{
		RecordingFinder finder = install(new RecordingFinder());

		tester.executeUrl("wicket/resource/" +
			ResourceUrlAttributeValidationTest.class.getName() + "/PublicPage.html?" +
			repeat("..%2F", PARENT_STEPS) + "otherpackage%2Fconfig");
	 ...
	 */
	@Test
	public void resourceResolvingOutsideThePackageIsNotServed()
	{
		install(new NormalizingFinder());

		tester.executeUrl(URL);

		assertFalse("a resource resolved outside the package must not be served",
			tester.getLastResponseAsString().contains(OUTSIDE_CONTENT));
	}
}

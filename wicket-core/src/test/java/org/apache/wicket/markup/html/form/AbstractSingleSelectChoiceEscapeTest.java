 ...
 */
public class AbstractSingleSelectChoiceEscapeTest extends WicketTestCase
{
	...
	@Test
	public void nullValidDisplayValueIsEscaped()
	{
		tester.startPage(new ChoicePage(MARKUP, true));

		String response = tester.getLastResponseAsString();
		assertTrue("the default option body should be escaped",
			response.contains(ESCAPED));
		assertFalse("the default option body should not reach the markup as markup",
			response.contains(MARKUP));
	}

	/** {@code nullValid} is false and nothing is selected, so the "null" display value is used. */
	@Test
	public void nullKeyDisplayValueIsEscaped()
	{
	 ...
	 */
	@Test
	public void defaultOptionIsNotEscapedWhenEscapeModelStringsIsFalse()
	{
		ChoicePage page = new ChoicePage(MARKUP, true);
		page.choice.setEscapeModelStrings(false);
		tester.startPage(page);

		assertTrue("escaping is off, so the default option body should be written as is",
	 ...
	 */
	@Test
	public void escapeOptionHtmlIsUsedForTheDefaultOption()
	{
		tester.startPage(new ChoicePage(MARKUP, true));
		...
		assertTrue("the default option should be escaped through escapeOptionHtml",
	 ...
	 */
	@Test
	public void displayValueHooksAreNotEscapedThemselves()
	{
		ChoicePage page = new ChoicePage(MARKUP, true);
		tester.startPage(page);

		assertEquals("the display value hook should see and return unescaped text",
			MARKUP, page.choice.observedDisplayValue);
		assertTrue("while the rendered option body is escaped",
			tester.getLastResponseAsString().contains(ESCAPED));
	}

	/** Plain text, which is what the shipped bundles hold, renders unchanged. */
	@Test
	public void plainDisplayValueRendersUnchanged()
	{
		tester.startPage(new ChoicePage("Choose one", true));

		assertTrue("plain text should pass through the escaping unchanged",
			tester.getLastResponseAsString().contains("value=\"\">[Choose one]</option>"));
	}
}

 ...
 */
package com.voicetribe.wicket.protocol.http.documentvalidation;
...
/**
 * Simple class that provides a convenient programmatic way to define what
 * an expected HTML document should look like and then to validate
 * a supplied document against this template. Note that this validator
 * expects very clean HTML (which should not be a problem during testing).
 * In particular it expects tags to be matched and that the following
...
public class HtmlDocumentValidator {
    ...
    /**
     * Add a root element to the validator. This will generally
     * be the HTML element to which all children are added. However,
     * it may also be other elements to represent comments or similar.
     *
    ...
    /**
     * Set whether to skip comments of not when validating. The default
     * is true. If this is set to false then Comment elements must be
     * added to represent each comment to be validated.
     *
    ...
    /**
     * Check whether the supplied document is valid against the
     * spec that has been built up within the validator.
     *
     ...
     */
    private Iterator validateCloseTag(Stack tagNameStack,
                                      HtmlDocumentParser parser,
                                      Iterator expectedElements,
                                      Stack iteratorStack) {
        if ( tagNameStack.isEmpty() ) {
            ...
            if ( !expectedTag.equals(parser.getTag()) ) {
                System.err.println("Found closing tag </" + parser.getTag() + "> when we expecting " +
     ...
     */
    private Iterator saveOpenTagState(Stack iteratorStack, Iterator expectedElements, Stack tagNameStack) {
    ...
    /**
     * Check if the supplied tag is one that expects to be closed or
     * not.
     *
     * @param tag The tag
    ...
    private boolean isNonClosedTag(String tag) {
        ...
        if ( tag.equals("area") ) return true;
        if ( tag.equals("base") ) return true;
        if ( tag.equals("basefont") ) return true;
        if ( tag.equals("bgsound") ) return true;
        if ( tag.equals("br") ) return true;
        if ( tag.equals("col") ) return true;
        if ( tag.equals("frame") ) return true;
        if ( tag.equals("hr") ) return true;
        if ( tag.equals("img") ) return true;
        if ( tag.equals("input") ) return true;
        if ( tag.equals("isindex") ) return true;
        if ( tag.equals("keygen") ) return true;
        if ( tag.equals("link") ) return true;
        if ( tag.equals("meta") ) return true;
        if ( tag.equals("param") ) return true;
        if ( tag.equals("spacer") ) return true;
        if ( tag.equals("wbr") ) return true;
        return false;
    }
            ...
            if ( expectedElements.hasNext() ) {
                ...
                if ( e instanceof Comment ) {
                    if ( !((Comment)e).getText().equals(parser.getComment() ) ) {
                        System.err.println("Found comment '" + parser.getComment() + "' does not match " +

...
public final class SerializableChecker extends ObjectOutputStream
{
	...
	static
	{
		try
		{
			...
			getNumObjFields = ObjectStreamClass.class.getDeclaredMethod("getNumObjFields",
					(Class[])null);
			...
			fieldMethod = ObjectStreamField.class.getDeclaredMethod("getField", (Class[])null);
	...
	private Map<Class<?>, Object> writeObjectMethodCache = new HashMap<Class<?>, Object>();
	...
	private void checkFields(Object obj, ObjectStreamClass desc)
	{
		...
		try
		{
			numFields = ((Integer)getNumObjFields.invoke(desc, (Object[])null)).intValue();
		}
		catch (IllegalAccessException e)
		{
			throw new RuntimeException(e);
		...
		if (numFields > 0)
		{
			...
			for (int i = 0; i < objVals.length; i++)
			{
				...
				try
				{
					field = (Field)fieldMethod.invoke(fieldDesc, new Object[] {});
				}
				catch (IllegalAccessException e)
				{
					throw new RuntimeException(e);

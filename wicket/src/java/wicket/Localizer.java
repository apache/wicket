...
/**
 * Utility class that encapsulates all of the localization related
 * functionality in a way that is can be accessed by all areas of the
 * framework in a consistent way. A singleton instance of this class
 * is available via the <code>ApplicationSettings</code> object.
 *
 * @author Chris Turner
...
public class Localizer {
     ...
     * information contained within the supplied settings object.
     *
     ...
     * This method is left public to allow developers full control over
     * string resource loading. However, it is recommended that one
     * of the other convenience methods in the class are used as they
     * handle all of the work related to obtaining the current user
     * locale and style information.
     *
     ...
     * @throws MissingResourceException If resource not found and configuration dictates that
    ...
    throws MissingResourceException {
        ...
        // Resource not found, so handle missing resources based on application configuration
     ...
     * null in which case the component string resource loader will not
     * be used. It the component is not null then it must be a component
     * that has already been added to a page, either directly or via a
     * parent container. The locale and style are obtained from the
    ...
    throws MissingResourceException {
        ...
        return getString(key, component, null, session.getLocale(),
                         session.getStyle(), defaultValue);
    }
     ...
     * that has already been added to a page, either directly or via a
     * parent container. The locale and style are obtained from the
     * current user session. If the model is not null then OGNL
     * substitution will be carried out on the string, using the
     * object contained within the model.
     *
     * @param key The key to obtain the resource for
     * @param component The component to get the resource for (optional)
     * @param model The model to use for OGNL substitutions in the strings (optional)
    ...
    throws MissingResourceException {
        Session session = Session.get();
        return getString(key, component, model, session.getLocale(),
                         session.getStyle(), defaultValue);
    }
    ...
    /**
     * Get the localized string. This method does not take a component
     * instance and hence the component string resource loader will
     * not be used when looking for the string resource.
     * The locale and style are obtained from the
     * current user session.
     *
     * @param key The key to obtain the resource for
     * @param defaultValue The default value (optional)
    ...
    throws MissingResourceException {
        Session session = Session.get();
        return getString(key, null, null, session.getLocale(),
                         session.getStyle(), defaultValue);
    }
    ...
    throws MissingResourceException {
        Session session = Session.get();
        return getString(key, null, model, session.getLocale(),
                         session.getStyle(), defaultValue);
    }

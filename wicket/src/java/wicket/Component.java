...
{ // TODO finalize javadoc
	 ...
	 * @return The application settings from the application that this component belongs
	 *         to
	...
	/**
	 * Convenience method to provide easy access to the localizer object within any
	 * component.
	...
	public final String getModelObjectAsString()
	{
		...
		if (mod != null)
		{
			...
			if (property != null)
			{
				...
				final String modelString = (property instanceof String) ? (String) property
	 ...
	 * @return The number of times this component has been rendered. This can be useful in
	 *         debugging.
	 */
	...
	final void checkRendering(final Page page)
	{
		page.visitChildren(new IVisitor()
		{
			public Object component(final Component component)
			{
				...
				{
					// Throw exception
					throw new RenderException(component
		...
		page.visitChildren(new IVisitor()
		{
			public Object component(final Component component)
			{
				...
				if (component.attributeModifiers != null)
				{
					...
					{
						IModel modifierModel = (IModel) ((ComponentTagAttributeModifier) it.next())
								.getReplaceModel();
	 ...
	 * @throws MarkupException Thrown if the tag does not have the required attribute
	 *             value
	 */
		...
		if (key != null)
		{
			...
			if (tagAttributeValue == null || !value.equalsIgnoreCase(tagAttributeValue))
			{
				findMarkupStream().throwMarkupException(
						"Component "
								+ getName() + " must be applied to a tag with '" + key
	...
	protected final void checkTag(final ComponentTag tag, final String name)
	{
		if (!tag.getName().equalsIgnoreCase(name))
		{
			findMarkupStream().throwMarkupException(
					"Component "
							+ getName() + " must be applied to a tag of type '" + name + "', not "
	 ...
	 * session associated with this component. Within each page in the session, traverses
	 * all components looking for a component attached to the same model and having the
	 * same name as this component. For each such model, the corresponding page is made
	 * stale. In addition, all previous renderings of the page holding this component are
	 * made stale.
	 */
					...
					currentPage.visitChildren(new IVisitor()
					{
						public Object component(final Component current)
						{
							...
							if (current.getModel() != null
									&& getModel() != null && current.getModel().equals(getModel())

...
public class WebRequestCycle extends RequestCycle
{
	...
	/**
	 * Return value for doParseRequest: The URL points to a page and the
	 * response page was set
	 */
	...
	protected final boolean parseRequest()
	{
		...
		else
		{
			...
			{
				// not found... send 404 to client indicating that no resource
				// was found
				// for the request uri
				...
				{
					// that seems unlikely... anyway, log exception and forget
					// about it
					log.error("unable to send 404 for " + getRequest() + ", cause: "
							+ e.getMessage(), e);
				}
			}
		}
	 ...
	 * WebRequestCrawlerSave and WebResponseCrawlerSave for examples on how to
	 ...
	 * images from a database without the need to register the resource with the
	 * application, or ...
	 * 
	 * @see WebRequestCrawlerSave
	 * @see WebResponseCrawlerSave
	 * 
	 * @return PARSE_REQUEST_RETURNING_PAGE - returning a page;
	 *         PARSE_REQUEST_RETURNING_STATIC_CONTENT - static content (no
	 *         Page); else - not found
	 */
	...
	protected void redirectTo(final Page page) throws ServletException
	{
				...
				&& (application instanceof WebApplication))
		{
			...
			try
			{
				...
				{
					// set it to null because it is already ended in the
					// page.doRender()
					setInvokePage(null);
				}
	...
	private boolean bookmarkablePage()
	{
		...
		if (bookmarkableName != null)
		{
			...
			// nope, we don't have a logical mapping, so this should be a full
			// class name
			if (pageClass == null)
			{
				try
				...
				catch (RuntimeException e)
				{
					...
					{
						// that seems unlikely... anyway, log exception and
						// forget about it
						log.error("unable to send 404 for " + getRequest() + ", cause: "
								+ ex.getMessage(), ex);
						return false;
			...
			try
			{
				...
				// If response is set in the construtor of the bookmarkable
				// page.
	...
	/**
	 * Calls a dispatched component listener interface on a page that already
	 * exists in the session. This is the same as callComponentListener, except
	 * that the actual handler is not the component itself, but an attached
	 * 'even request listener'. Such listeners (typically used for AJAX
	 * behaviour) are responsible for their own output (which could be XML,
	 * javascript, HTML or whatever), and thus the current page should not be
	 * rendered when this method returns true.
	 * 
			...
			try
			{
				...
				else
				{
					...
					if (getResponsePage() == null)
					{
						...
						{
							// see if we have to redirect the render part by
							// default
							// so that a homepage has the same url as a post or
							// get to that page.
							ApplicationSettings.RenderStrategy strategy = getSession()
									.getApplication().getSettings().getRenderStrategy();
	...
	private void invokeInterface(final Page page, final String path, final String interfaceName)
	{
		...
		if (component != null)
		{
			if (!component.isVisible())
			{
				...
				{
					getWebResponse().getHttpServletResponse().sendError(
							HttpServletResponse.SC_FORBIDDEN, "Unable to execute this request");
				}
				catch (IOException ex)
				{
					// that seems unlikely... anyway, log exception and forget
					// about it
					log.error("unable to send 403 for " + getRequest() + ", cause: "
							+ ex.getMessage(), ex);
				}
				return;
			}
		...
		if (path.startsWith(resourceReferencePrefix))
		{
			...
			if (resource == null)
			{
				...
				catch (IOException ex)
				{
					...
					throw new WicketRuntimeException("Could not find resource referenced by key "
							+ rawResourceKey + " and send a 404", ex);
				}
	...
	private boolean staticContent()
	{
		try
		{
			...
			// of
			// application servers (e.g. Jetty 5.1.x) will fail for requests
			// like
			// '/mysubdir/myfile.css'

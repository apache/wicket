...
public class MockHttpServletResponse implements HttpServletResponse {
    ...
    /**
     * Encode thr URL. Does no changes as this test implementation
     * uses cookie based url tracking.
     *
    ...
    /**
     * Encode the redirectLocation URL. Does no changes as this test implementation
     * uses cookie based url tracking.
     *
     * @param url The url to encode
     * @return The encoded url
    ...
    /**
     * Send an error code. This implementation just sets the internal
     ...
     * This implementation just keeps hold of the redirectLocation info and makes
     * it available for query.
     *
    ...
    /**
     * Add the given header value, including an additional entry
    ...
    public String getDocument() {
        if ( mode == MODE_BINARY ) return new String(byteStream.toByteArray());
        else return stringWriter.getBuffer().toString();
    }
    ...
    public int getBufferSize() {
        ...
        else if ( mode == MODE_BINARY ) return byteStream.size();
        else return stringWriter.getBuffer().length();
    }
    ...
    public void resetBuffer() {
        if ( mode == MODE_BINARY ) byteStream.reset();
        else if ( mode == MODE_TEXT ) stringWriter.getBuffer().delete(0, stringWriter.getBuffer().length());
    }

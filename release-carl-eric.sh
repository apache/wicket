...
find . -name "pom.xml" | xargs sed -i -e "s/1.4-SNAPSHOT/$version/g"
...
ssh cmenzel@people.apache.org mkdir -p dist/wicket-$version public_html/wicket-$version
scp -r target/dist cmenzel@people.apache.org:dist/wicket-$version
scp -r $gitarchive* cmenzel@people.apache.org:public_html/wicket-$version

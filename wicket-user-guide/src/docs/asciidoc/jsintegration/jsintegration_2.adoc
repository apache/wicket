...
image::datepicker-package-resources.png[]
...
To generate the relative URL for calendar icon, we load it as package resource reference and then we use 'Component''s method urlFor to get the URL value (we have seen this method in  <<guide:requestProcessing_3,paragraph 9.3.2>>
).
...
WARNING: Our datepicker is not ready yet to be used with AJAX. In  <<guide:ajax,chapter 19>>

...
Once we have our listener in place, we can finally use it in our example overriding method _updateAjaxAttributes_ of the AJAX button that submits the form:
...
The callback function takes in input the following parameters:  attrs, jqXHR, textStatus, jqEvent and errorThrown. The first three parameters are the same seen before with _IAjaxCallListener_ while jqEvent is a lightweight event object internally fired by Wicket to notify subscribers (its shape depends on which `wicket-ajax.js` implementation is configured, see <<_choosing_between_the_jquery_and_the_plain_javascript_ajax_implementation>>). The last parameter errorThrown indicates if an error has occurred during the AJAX call.

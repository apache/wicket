...
From Wicket 11 it will also process the inner components of _FormComponentPanels_( regardless of the kind of descendant components). This ensures the model of the _FormComponentPanel_ has been updated before _onUpdate(...)_ is invoked. Note that this will force recursive serialization of the data of the inner components. To restore the pre-Wicket 11 behavior return _false_ from _updateInnerComponents()_.

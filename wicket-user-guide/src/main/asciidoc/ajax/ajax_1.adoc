...
* Register the replacement method with Wicket AJAX. This is usually handled by a behavior added to the component for which to use the alternative method. If the method has not been registered when the AJAX response is processed (and Wicket AJAX debug mode is enabled), an error will be logged.
...
* XML: for replacing markup of SVG and MathML. See _XmlReplacementEnablingBehavior_.

...
To customize the set of _IMarkupFilters_ used in our application we can create a subclass of _MarkupFactory_ overriding method _newMarkupParser(MarkupResourceStream resource)_:
...
Usually we won't need to change the default configuration of _IMarkupFilters_, but it's important to be aware of this internal mechanism before we talk about another advanced feature, which is building auto components resolvers.

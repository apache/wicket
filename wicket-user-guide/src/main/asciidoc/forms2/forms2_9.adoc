...
The Kelvin scale is widely adopted among the scientific community and it is one of the seven base units of the  http://en.wikipedia.org/wiki/International_System_of_Units[International System of Units] , so it makes perfect sense to store temperatures expressed with this unit of measurement.
...
* Add Ajax behaviors to 1 or more of the child components. This allows fine-grained updates, but during an Ajax request only the updated value of the child component is available. You will have to update the model of the panel using the child value manually.
...
This method is a bit more cumbersome than Ajax on the panel itself, and requires a bit more code. But fewer requests will be made, and the requests will have the smallest possible size.
...
var ajaxOnDescendents = new Palette<>("paletteOnDescendents", descendentsModel, new CollectionModel<>(persons), renderer, 10, true, true)
{
    ...
    protected Recorder<Person> newRecorderComponent()
    {
        ...
        recorder.add(new AjaxFormComponentUpdatingBehavior("change")
        {
            ...
            {
                processInput();
                // The model will now contain the correct people.
            }
        });
        return recorder;
    }
...
With this method adding Ajax behavior to a panel works just like with simple form fields. Ajax can be used with less code than Ajax on child components. But the requests will be larger because data of all children will be sent with each request. And in some cases (and unfortunately) requests will be sent for more JavaScript events, even if there was no change in the form data.

...
public class PageLinkPage extends WicketExamplePage
{
    ...
    public PageLinkPage()
    {
    	...
    	//
    	// The disadvantage of that is that you need to create an instance right away, which
    	// will be kept as session data. Instead, we use the more verbose IPageLink
    	// anonymous class, which will create the page instance only when needed.
    	...
    	// Note that this would have had the same effect, except that the link wouldn't check
		// whether it points to the current page and thus should be 'turned off' (but actually
		// we don't need that check here, as we are certain that is not the case)
		//

...
class CSPHeaderWriterTest extends WicketTestCase
{
	...
	@Test
	void addCSPDirectiveToStatelessPage()
	{
		tester.startPage(Page.class);

		assertTrue(tester.getLastResponse().containsHeader(CSPHeaderMode.BLOCKING.getHeader()));
	}

	@Test
	void dontAddCSPDirectiveToStatelessPage()
	{
		tester.getApplication().getCspSettings().setProtectedFilter(p -> !(p instanceof Page));
		tester.startPage(Page.class);

		assertFalse(tester.getLastResponse().containsHeader(CSPHeaderMode.BLOCKING.getHeader()));
	}
